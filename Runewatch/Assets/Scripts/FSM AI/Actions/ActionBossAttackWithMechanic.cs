...
/// </summary>
[CreateAssetMenu(menuName = "FSM/Actions/Action Random Boss Attack w Mechanic", 
    fileName = "Action Random Boss Attack w Mechanic")]
sealed public class ActionBossAttackWithMechanic : FSMAction
{
    ...
    private void Attack(StateController<Enemy> ai)
    {
        ...
        {
            EnemyBoss boss = ai.Controller as EnemyBoss;
                ...
                ai.Controller.CurrentlySelectedSpell.StoppingTime)
            {
                if (ai.Controller.CurrentlySelectedSpell.Spell.CastType == SpellCastType.OneShotCastWithRelease)
                {
                    ...
                    // After attacking with special mechanic, sets mechanic executed to true
                    if (boss != null)
                    {
                        if (boss.ExecuteFirstMechanic &&
                            ai.Controller.CurrentlySelectedSpell.Spell.Name ==
                            ai.Controller.EnemyStats.EnemyAttributes.SpecialMechanic.Spell.Name)
                        {
                            boss.ExecutedFirstMechanic = true;
                        }

                        if (boss.ExecuteSecondMechanic &&
                            ai.Controller.CurrentlySelectedSpell.Spell.Name ==
                            ai.Controller.EnemyStats.EnemyAttributes.SpecialMechanic.Spell.Name)
                        {
                            boss.ExecutedSecondMechanic = true;
                        }
                    }
                }
                else
                    ...
                    if (boss != null)
                    {
                        if (boss.ExecuteFirstMechanic && boss.ExecutedFirstMechanic == false)
                        {
                            ai.Controller.CurrentlySelectedSpell =
                                ai.Controller.EnemyStats.EnemyAttributes.SpecialMechanic;
                        }
